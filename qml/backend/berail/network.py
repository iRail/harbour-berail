...
@title: Network class
@description: Network class for BeRail to handle all the network requests and connections with ConnMan.
@test: test/network_test.py
"""

#BeRail modules
from berail import constants, sfos, logger
...
class _Connection(object):
    ...
    """   
    def send(self, url, payload=None, http_type=constants.http.TYPE["GET"], session=True, host=constants.irail.HOST, files=None, raw=False, wait=True):
        limit_execution = 50
        ...
        if requests.get(constants.http.TEST["IPV4"]).status_code == constants.http.SUCCESS["OK"] or requests.get(constants.http.TEST["IPV6"]).status_code == constants.http.SUCCESS["OK"]: # Check connection
            ...
            if session:
                ...
                self._session.headers.update({"User-Agent" : constants.berail.agent})
